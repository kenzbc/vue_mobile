<template>
  <div class="anim-opacity2">
    <div class="match_area-head" v-if="newRoomInfo">
        <div @click="openApp()" v-show="shareType1=='app'" class="open-app">
            <a :href="linkUrl">App内打开</a>
        </div>
    ...
    <!--弹框 -->
    <dialog-exchange v-if="exchangeShow" @dialogHide="change" :msg="exchangeShow" :exchange-list="exchangeData"></dialog-exchange>



    <!--新手引导弹层-->
    <div v-if="courseStep == '13'" class="tutorial4">
      <div class="content">
        <div class="head-subtitle">
          <div class="item"></div>
          <div class="item"></div>
          <div class="item" @click="newUserNextBtn('13')"><div class="select-btn">挑选阵容</div></div>
        </div>
        <div class="desc-text">
...
import Users from "../components/room/users.vue";
...
export default {
  ...
  created() {
    ...
    this.$root.hub.$on("loginSuccess", _ => {
      ...
      if(store.get('member_info').is_new_user == '1'){
        ...
        if (this.$route.query.experience && get_new_player.indexOf(this.$route.path) < 0) {
  ...
  methods: {
    newUserNextBtn(step){
      if(step=='13'){
        store.set("new_user_step",'14');//下一步骤14
        this.courseStep = '14';
        this.setNewUserTask(4);//设置第4阶段完成
        this.matchBtn(this.newRoomInfo.ticket,this.newRoomInfo.price)
      }
